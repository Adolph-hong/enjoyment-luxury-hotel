  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        // ✅ TypeScript 現在知道 error 有 message 屬性
        if (error.message.includes('未填寫')) {
          alert(
        ...
        // ✅ 若 error 不是 Error 類型，也安全處理
        alert(String(error) || '訂房失敗，請稍後再試')
      }
    } finally {
      setIsBooking(false)
    }
            ...
            <div className="bg-white rounded-[20px] p-8 sticky top-32">
              ...
              <div className="mb-6 pb-6 border-b border-[#ECECEC]">
                <div className="flex justify-between mb-2">
                  ...
                  <span className="text-[#140F0A] font-medium">{checkInDate || '請選擇日期'}</span>
                </div>
                <div className="flex justify-between mb-2">
                  <span className="text-[#4B4B4B]">退房日期</span>
                  <span className="text-[#140F0A] font-medium">{checkOutDate || '請選擇日期'}</span>
                </div>
                <div className="flex justify-between">
